package {
	...
	public class FLxER6_1 extends Sprite {
		...
		private var version							:Number = 6.1;
		public function FLxER6_1() {
			...
			if (loadDefaults) {
				trace("loadDefaults "+loadDefaults);
				...
				Preferences.pref = myPrefSO.data.pref = JSON.parse(json);
		...
		private function interfaceDrawer():void {
			...
			monitor = new Monitor(300, 23+((300-Preferences.monObj.monHeight)/2), Preferences.monObj.monWidth, Preferences.monObj.monHeight, Preferences.pref.nCh);
		...
		*/
		public function resizer(event:Event):void {
			Preferences.monObj.monWidth = outWindow.stage.stageWidth/(Preferences.stereo ? 2 : 1);
			Preferences.monObj.monHeight = outWindow.stage.stageHeight;
			monitor.resizer();
			if (monitorOut) monitorOut.resizer();
			var ddd:Number;
			var www:int;
			var hhh:int;
			if (Preferences.monObj.monWidth/Preferences.monObj.monHeight > 1000/300) {
				trace("stocazzo1")
				ddd = Preferences.monObj.monWidth/1000;
				www = 1000;
				hhh = int(Preferences.monObj.monHeight/ddd);
			} else {
				ddd = Preferences.monObj.monHeight/300;
				www = int(Preferences.monObj.monWidth/ddd);
				hhh = 300
				trace("wwwww "+Preferences.monObj.monWidth);
				trace("hhhhh "+Preferences.monObj.monHeight);
				trace("stocazzo2"+ddd)
			}
		...
		private function goFullKey(event:Event):void {
		...
		public function myFullscreen(p:Boolean,stereo:Boolean):void {
			...
			if (p) {
				...
				if (stereo) {
					trace("myfull"+stereo);
					outWindow.bounds = new Rectangle(0, 0, Preferences.monObj.monWidth*2,Preferences.monObj.monHeight);
					monitorOut = new OutputStereo(0, 0, Preferences.monObj.monWidth,Preferences.monObj.monHeight, Preferences.pref.nCh);
				} else {
					outWindow.bounds = new Rectangle(0, 0, Preferences.monObj.monWidth,Preferences.monObj.monHeight);
					if (Preferences.pref.bitmap) {
						monitorOut = new OutputBMP(Preferences.monObj.monWidth,Preferences.monObj.monHeight);
					} else {
						monitorOut = new Output(0, 0, Preferences.monObj.monWidth,Preferences.monObj.monHeight, Preferences.pref.nCh);
					}
				}
		...
		public function openPrefPanel():void {
			if (!myPrefOption) myPrefOption = new PrefOption(400, 395, "FLXER PREFERENCES", savePref,closePrefPanel);
		}
